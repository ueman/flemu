import 'package:gb_emulator/sound/abstract_sound_mode.dart';
import 'package:gb_emulator/sound/frequency_sweep.dart';
import 'package:gb_emulator/sound/volume_envelope.dart';

class SoundMode1 extends AbstractSoundMode {
  int _freqDivider = 0;

  int _lastOutput = 0;

  int _i = 0;

  final FrequencySweep _frequencySweep = FrequencySweep();

  final VolumeEnvelope _volumeEnvelope = VolumeEnvelope();

  SoundMode1(bool gbc) : super(0xff10, 64, gbc);

  @override
  void start() {
    _i = 0;
    if (gbc) {
      length.reset();
    }
    length.start();
    _frequencySweep.start();
    _volumeEnvelope.start();
  }

  @override
  void trigger() {
    _i = 0;
    _freqDivider = 1;
    _volumeEnvelope.trigger();
  }

  @override
  int tick() {
    _volumeEnvelope.tick();

    bool e = true;
    e = updateLength() && e;
    e = _updateSweep() && e;
    e = dacEnabled && e;
    if (!e) {
      return 0;
    }

    if (--_freqDivider == 0) {
      _resetFreqDivider();
      _lastOutput = ((_getDuty() & (1 << _i)) >> _i);
      _i = (_i + 1) % 8;
    }
    return _lastOutput * _volumeEnvelope.getVolume();
  }

  @override
  void setNr0(int value) {
    super.setNr0(value);
    _frequencySweep.setNr10(value);
  }

  @override
  void setNr1(int value) {
    super.setNr1(value);
    length.setLength(64 - (value & 63 /* 0b00111111 */));
  }

  @override
  void setNr2(int value) {
    super.setNr2(value);
    _volumeEnvelope.setNr2(value);
    dacEnabled = (value & 247 /* 0b11111000 */) != 0;
    channelEnabled &= dacEnabled;
  }

  @override
  void setNr3(int value) {
    super.setNr3(value);
    _frequencySweep.setNr13(value);
  }

  @override
  void setNr4(int value) {
    super.setNr4(value);
    _frequencySweep.setNr14(value);
  }

  @override
  int getNr3() {
    return _frequencySweep.getNr13();
  }

  @override
  int getNr4() {
    return (super.getNr4() & 248 /*0b11111000 */) |
        (_frequencySweep.getNr14() & 7 /* 0b00000111 */);
  }

  int _getDuty() {
    switch (getNr1() >> 6) {
      case 0:
        return 1 /* 0b00000001 */;
      case 1:
        return 129 /* 0b10000001 */;
      case 2:
        return 135 /* 0b10000111 */;
      case 3:
        return 126 /* 0b01111110 */;
      default:
        throw Exception('IllegalState');
    }
  }

  void _resetFreqDivider() {
    _freqDivider = getFrequency() * 4;
  }

  bool _updateSweep() {
    _frequencySweep.tick();
    if (channelEnabled && !_frequencySweep.isEnabled()) {
      channelEnabled = false;
    }
    return channelEnabled;
  }
}
